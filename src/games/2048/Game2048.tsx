...
import { createAndSaveGame, loadBestScore, loadCurrentGame, saveBestScore, saveCurrentGame, type Game2048Save } from './storage'
import { readJSON, STORAGE_KEYS, updateStats } from '../../storage'

type GameState = Game2048Save & { best: number }
const initial = (): GameState => ({ ...loadCurrentGame(), best: loadBestScore() })
export function Game2048({ onBack }: { onBack: () => void }) {
  ...
  const commitGame = (next: GameState) => {
    ...
    try {
      ...
      if (best > loadBestScore()) saveBestScore(best)
    } catch { /* Storage can be unavailable without stopping the active game. */ }
    setGame(next)
  }
  const act = (direction: Direction) => {
    ...
    const next = { version: 1 as const, board, score, best, won: current.won || wonNow, over }
    commitGame(next)
    return next
  }
  ...
  const restart = () => { if (!confirm('确定重新开始？这一局的进度会消失。')) return; const best = gameRef.current.best; const fresh = createAndSaveGame(); gameRef.current = { ...fresh, best }; setGame(gameRef.current); setNotice(null); const stats = readJSON(STORAGE_KEYS.stats, { gamesPlayed: 0, wins: 0, recentGame: null }); updateStats({ gamesPlayed: stats.gamesPlayed + 1, recentGame: '2048' }) }
