...
import { GAME_2048_SAVE_VERSION, persistGame2048, readJSON, restoreGame2048, STORAGE_KEYS, updateStats } from '../../storage'
...
export function Game2048({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => { let active = true; void restoreGame2048(best => ({ ...restartedGame2048(best), updatedAt: Date.now() })).then(restored => { if (active) { setGame(restored); hydrated.current = true } }); return () => { active = false } }, [])
  const act = (direction: Direction) => setGame(current => {
    if (current.over || !hydrated.current) return current
    ...
    const next = { version: GAME_2048_SAVE_VERSION, board, score, best, won: current.won || wonNow, over, updatedAt: Date.now() }
    persistGame2048(next)
    return next
  })
  ...
  const restart = () => { if (!confirm('确定重新开始？这一局的进度会消失。')) return; const next = restartedGame2048(game.best); persistGame2048(next); setGame(next); setNotice(null); const stats = readJSON(STORAGE_KEYS.stats, { gamesPlayed: 0, wins: 0, recentGame: null }); updateStats({ gamesPlayed: stats.gamesPlayed + 1, recentGame: '2048' }) }
